...
def _load_sbm(modelopts):
    ...
    print "inputs.py: prima di SBM"
    print "passing also ",modelopts["atomtype_file"]
    ...
    #Check for nb interactions defined through custom atom types
    if modelopts["atomtype_file"] is not None:
        model.atomtype_dictbyname = init_atmtype_dict()
        model.atomtype_dictbyname = parse_atmtypes_file(modelopts["atomtype_file"], model.atomtype_dictbyname)
        model.assign_types(model.atomtype_dictbyname)
    else:
        model.atomtype_dictbyname = init_atmtype_dict()
        model.assign_types(model.atomtype_dictbyname)
...
def _empty_model_opts():
    ...
    opts = ["pairs_file","pairwise_params_file",
            ...
            "defaults","bead_repr","atomtype_file","cb_volume","disulfides",
...
def init_atmtype_dict():
        """ The dictionary contains a certain number of entries. The dict
        key is prefx + X where prefx=CA and X=one letter code of amino acids.
        Any other kind of atom type definition could be added in the future
        into the dictionary.
        """
...
def parse_atmtypes_file(atomtype_file, atomtype_dict):
        """ Parse the file containing the atomtypes parameters and populates
        the dictionary atomtype_dictbyname with them.

        atomtype_file should have the same format [ atomtypes ] section 
        in .top of gromacs (name, mass, charge, ptype, c6, c12).
        ...
        atomtype_dictbyname = atomtype_dict

        # here I populate the dictionary. To note: the initialization above, i.e. an empty list
        # implies that atomtype_dictbyname[val] is False. I use this property below:

        #r0 is in nm and eps in kJ.

        fil = open(atomtype_file, "r")
        for line in fil:
            entry = line.split()
            r0 = float(entry[0]) 
            eps = float(entry[1])
            ...
            if eps > 0.00001:
                ...
                c6 = 0.0
                c12 = r0**12
            if name in atomtype_dictbyname:
                if not atomtype_dictbyname[name]:
                    atomtype_dictbyname[name] = [mass, charge, ptype, c6, c12]

                else:
                    print "\nWarning The input file ",atomtype_file," contains duplicates. The first occurrence will be used in the .top file"
            else:
                raise IOError("Unknown atom type {}. Available atom types are {}".format(name, atomtype_dictbyname.keys()))

        return atomtype_dictbyname

def parse_atmtypes_file_old(atomtype_file, atomtype_dict):
        """ Parse the file containing the atomtypes parameters and populates
        the dictionary atomtype_dictbyname with them.
