...
class CalphaMapping(object):
    ...
    def add_atoms2(self):
        print "calpha.py function add_atoms2: "

        prefx = 'CA'
        self.atoms = []
        for atom in self.top.atoms:
            cg_atom = atom_types.CoarseGrainAtom(atom.index, prefx, 
    ...
    def add_atoms(self, mass=1, radius=0.4, charge=0):
        ...
        for atom in self.top.atoms:
            ...
            self.atoms.append(cg_atom)
            if atom.index == 1 :
                print cg_atom.name, cg_atom.resname, cg_atom.radius, cg_atom.c12

            # Unique list of atom types.
            if cg_atom.name not in [ atm.name for atm in self.atomtypes ]:
                self.atomtypes.append(cg_atom)

    def add_atmtypes(self, atmtype_dict):
        self.atomtypes = []
        n_atm_types = 0
        for key in atmtype_dict:
            if atmtype_dict[key]:
                n_atm_types += 1 
                atp = atom_types.CoarseGrainAtomType(key, atmtype_dict[key])
                self.atomtypes.append(atp)

        if n_atm_types == 0:
            radius = 4.0
            c12 = (radius)**12
            c6 = 0.0
            atp = atom_types.CoarseGrainAtomType("CA", [1.000, 0.000, 'A', c6, c12])
            self.atomtypes.append(atp)
